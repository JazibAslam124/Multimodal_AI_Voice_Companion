...
# memory.py - Lena's long-term memory using ChromaDB
...
class MemoryManager:
    ...
    def add_turn(self, user_text: str, ai_text: str):
        ...
        try:
            ...
            print(f"   [Memory] Failed to add turn: {e}")

    def store_facts(self, facts: list):
        """Stores extracted facts from memory_extractor."""
        for fact in facts:
            try:
                text = fact.get("fact", "").strip()
                if not text:
                    continue
                fact_id = f"user::{fact.get('predicate', 'fact')}::{uuid.uuid4().hex[:8]}"
                if fact.get("memory_type") == "profile_fact":
                    fact_id = f"user::{fact.get('predicate', 'fact')}"

                meta = {
                    "type": fact.get("memory_type", "episodic"),
    ...
    def get_context(self, query_text: str, n_results: int = 4) -> str:
        ...
        try:
            ...
            if results and results["documents"] and results["documents"][0]:
                facts = list(set(results["documents"][0]))
                return "What you know about your friend: " + " | ".join(facts)
            return ""
        except Exception as e:
    ...
    def get_recent_facts(self, limit: int = 5) -> list:
        """Returns the most recently stored facts."""
        try:
            if self.facts.count() == 0:
                return []
            result = self.facts.get(include=["documents", "metadatas"])
            ...
            return [doc for doc, _ in zipped[:limit]]
        except Exception as e:
            print(f"   [Memory] Recent facts failed: {e}")
            return []